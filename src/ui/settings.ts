...
function ensureInfoTabsAndCards() {
  try {
    ...
    const ensureTab = (tabId: string, label: string) => {
      let btn = tabsRoot.querySelector<HTMLButtonElement>('[data-tab="' + tabId + '"]');
      if (!btn) {
    ...
    const ensureCard = (tabId: string, html: string) => {
      let card = body.querySelector<HTMLElement>('.settings-card[data-tab="' + tabId + '"]');
      if (!card) {
