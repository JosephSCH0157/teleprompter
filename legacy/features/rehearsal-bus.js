...
export function broadcastRehearsal(on) {
    const msg = { type: 'rehearsal', on, ts: Date.now() };
    ...
    try {
        const framesArr = Array.from(window.frames ?? []);
...
export function attachRehearsalListeners(handler) {
    // DOM
