...
- Commit-time safety clamp: before ASR writer/pixel commit finalization, bound forward index delta for rescue commits. `watchdog` and other forced commit reasons are clamped to a small fixed max delta (default `+2` lines) and emit a dev `ASR_CLAMP` line when applied.
