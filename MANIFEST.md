...
- Live armed ASR cue-stall auto-rescue is allowed only when the current cursor line is cue/meta/blank: when commit progress stalls while transcript traffic continues and cursor is cue-like, driver may auto-schedule the bounded cue-bridge forward commit (`forceReason='stall-rescue'`) with cooldown; content-line stalls must not auto-bridge.
...
- LOST_FORWARD may apply a bounded behind-marker catch-up when marker is ahead of cursor and forward content evidence exists: keep low floor (`sim>=0.20`) but cap progression per commit (`+2`) and keep candidate window bounded (`<=+6`).
...
- Writer commits must satisfy glide-aware scroll-truth: while writer glide/seek animation is active, defer desync verdict; after glide settles plus settle frames (`~3 rAF`), if actual viewer `scrollTop` mismatches expected target, run one hard correction via target-line `scrollIntoView({block:'center'})`; if still mismatched, emit `scroll_desync` and enter bounded resync without advancing commit cursor state.
...
- End-of-script completion in live armed ASR is explicit: when commit lands on the last speakable line, driver emits one-shot `tp:asr:script-end`, and runtime transitions session to `wrap` via session-intent stop (`active=false`) plus `tp:session:stop` to prevent EOF stall/restart loops.
