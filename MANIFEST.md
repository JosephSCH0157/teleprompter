...
- Live armed ASR cue-stall auto-rescue is allowed: when commit progress stalls while transcript traffic continues, driver may auto-schedule the same bounded cue-bridge forward commit (`forceReason='stall-rescue'`) with cooldown, without requiring an external rescue event.
