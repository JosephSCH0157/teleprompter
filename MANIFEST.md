...
- Logical starvation recovery must trigger early (before time-based starvation): when `lost_forward_gate` rejects and low-sim arbitration blocks immediate continuation (`delta=+1`) for the configured consecutive wall streak (`>=3`), attempt the same bounded forward relock immediately.
