...
- Add progress-based LOST_FORWARD recovery: track a no-progress streak per ingest (reset only when cursor advances). At streak `>=4`, enter temporary LOST_FORWARD mode (`+25` forward window, forward-only jump candidate, `sim>=0.45` floor) and exit/reset immediately on the next forward commit.
- ASR token normalization must recombine split tens+ones number tokens (for example `60 8 -> 68`) before matcher scoring so spoken number words do not degrade alignment confidence.
