...
- Live armed ASR cue-stall auto-rescue is bounded: when commit progress stalls while transcript traffic continues, driver may auto-schedule (a) bounded cue-bridge rescue when cursor is cue/meta/blank, or (b) conservative direct content rescue (`+1` line only) when cursor and target are both content lines.
