...
- Short-line ambiguity rule: when best candidate line is short, meaningful overlap is sparse (`<=2` content-token hits), confidence is weak/moderate, and runner-up is nearby (`±1..2`), enter HOLD immediately (no commit, no cue-bridge) and wait for stronger forward anchor evidence.
