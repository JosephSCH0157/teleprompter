...
- Bounded same-line final cue-bridge promotions may apply only a tiny multi-jump floor epsilon (`<=0.005`) to prevent float-boundary stalls; this is limited to `final-forward-nudge` / `final-forward-fallback` / `permissive-final-advance` with valid cue-bridge pathing, and does not change the hard deny floor (`sim<0.30`).
