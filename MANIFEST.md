...
- Writer commits must satisfy scroll-truth after settle (`~3 rAF`): if actual viewer `scrollTop` mismatches expected target, run one hard correction via target-line `scrollIntoView({block:'center'})`; if still mismatched, emit `scroll_desync` and enter bounded resync without advancing commit cursor state.
