...
- Writer commit scroll-truth is glide-aware and enforced: while writer glide/seek is active, defer desync verdict; after glide settles plus settle (`~3 rAF`), if viewer `scrollTop` does not match expected target, run one hard correction via target-line `scrollIntoView({block:'center'})`; if mismatch persists, emit `scroll_desync`, clear pending escalation state, and enter bounded resync without accepting that commit as cursor progress.
