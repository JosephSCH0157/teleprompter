            ...
            try {
                ...
                if (snr < THRESH) {
                    if (belowStart == null)
                        ...
                        shown = true;
                        window.toast?.('Input is noisy — calibrate?', { type: 'warn', actionLabel: 'Open', action: openAsrSettings });
                    }
                }
                else {
                    belowStart = null;
...
(function webspeechInstabilityWatcher() {
    try {
        ...
        window.addEventListener('tp:asr:error', (e) => {
            try {
                const code = String(e?.detail?.code || '');
                if (!code.startsWith('webspeech'))
                    return;
                const now = performance.now();
                if (firstTs == null || now - firstTs > WINDOW_MS) {
                    firstTs = now;
                    count = 1;
                    return;
                }
                count++;
                if (count >= 2) {
                    window.toast?.('Recognition unstable — calibrate or use VAD mode', { type: 'warn', actionLabel: 'Open', action: openAsrSettings });
