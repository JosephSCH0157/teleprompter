...
function openSettingsToAsr(startWizard) {
    try {
        // Open overlay
        window.__tpSettings?.open?.();
        // Switch to Media tab
        const overlay = document.getElementById('settingsOverlay');
        if (overlay)
            overlay.classList.remove('hidden');
        const tabsRoot = document.getElementById('settingsTabs') || document;
        tabsRoot?.querySelectorAll('[data-tab-content]')?.forEach((c) => (c.style.display = 'none'));
        const media = document.querySelector('[data-tab-content="media"]');
        if (media)
            media.style.display = '';
        ...
        setTimeout(() => {
            ...
            if (sec) {
                ...
                try {
                    sec.classList.add('asr-highlight');
                    setTimeout(() => sec.classList.remove('asr-highlight'), 1400);
                }
                catch { }
            }
            // focus device select if present
            const devSel = document.getElementById('asrDevice');
            devSel?.focus?.();
            if (startWizard) {
                try {
                    window.startAsrWizard?.();
                }
                catch { }
            }
        }, 50);
  ...
  `;
    el.addEventListener('click', (e) => {
        ...
        if (t.matches('.mm-item')) {
            ...
            if (act === 'grant') {
                try {
                    window.__tpMic?.requestMic?.();
                }
                catch { }
            }
            if (act === 'select')
                openSettingsToAsr(false);
            if (act === 'calibrate') {
                try {
                    const Auto = window.__tpAuto || window.Auto;
                    const isLive = !!Auto?.getState?.().enabled;
                    if (isLive) {
                        window.toast?.('Pause Auto-scroll to calibrate', { type: 'info' });
                    }
                    else {
            ...
            try {
                setActiveProfile(id);
                window.toast?.('ASR profile selected', { type: 'info' });
            }
            catch { }
            hide();
        }
...
function populateProfiles(el) {
    try {
        const cont = el.querySelector('#mm-profiles');
        if (!cont)
            return;
        cont.innerHTML = '';
        const s = getAsrState();
...
(function initMicPillMenu() {
    try {
        ...
        document.addEventListener('change', (e) => {
            const t = e.target;
            if (t?.id === 'settingsMicSel') {
                window.toast?.('Mic device changed — calibrate?', { type: 'warn', actionLabel: 'Open', action: () => openSettingsToAsr(false) });
            }
        }, { capture: true });
    }
    catch { }
...
(function firstRunToast() {
    try {
        ...
        if (!s.activeProfileId) {
            window.toast?.('ASR not calibrated — open ASR settings', {
                type: 'warn',
                actionLabel: 'Open',
