...
export function installEasterEggs(){
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (pos === konami.length){
      ...
      try { (window.setStatus||(()=>{}))(on ? 'Savanna unlocked 🦁' : 'Savanna off'); } catch {}
    }
  });
  ...
  if (meter){
    ...
    meter.addEventListener('click', () => {
      ...
      if (clicks >= 5){
        ...
        try { (window.setStatus||(()=>{}))(meter.classList.contains('party') ? 'Meter party 🎉' : 'Meter normal'); } catch {}
      }
    });
  }
  ...
  if (helpTitle && advanced){
    helpTitle.addEventListener('click', (e)=>{ if (e.altKey) advanced.classList.toggle('hidden'); });
  }
...
export function installCKEgg(){
  const enable = (silent=false) => {
    ...
    try { if (window.sendToDisplay) window.sendToDisplay({ type:'toggle-ck', on:true }); } catch {}
  ...
  const disable = (silent=false) => {
    ...
    try { if (window.sendToDisplay) window.sendToDisplay({ type:'toggle-ck', on:false }); } catch {}
